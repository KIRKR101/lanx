...
pub async fn run(relay: Option<String>, port: u16) -> Result<()> {
    ...
    if interfaces.is_empty() {
        ...
        println!("OK    interfaces: {}", interfaces.iter().map(ToString::to_string).collect::<Vec<_>>().join(", "));
    }

    check_tcp_port(port).await;
    match UdpSocket::bind(("0.0.0.0", lanx_net::discovery::DISCOVERY_PORT)).await {
        Ok(_) => println!("OK    UDP discovery port {} is available", lanx_net::discovery::DISCOVERY_PORT),
        Err(error) => println!("WARN  UDP discovery port {} unavailable: {error}", lanx_net::discovery::DISCOVERY_PORT),
    }
    check_writable(Path::new("."));

    if let Some(relay) = relay {
        let addr: SocketAddr = relay.parse().with_context(|| format!("invalid relay address: {relay}"))?;
